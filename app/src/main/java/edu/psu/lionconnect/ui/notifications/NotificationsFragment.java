...
import edu.psu.lionconnect.Settings;
...
//        });
        ...
        boolean changeBackground = prefs.getBoolean("background", true);
        if(changeBackground){
            root.getRootView().setBackgroundColor(Color.LTGRAY);
        }
        boolean forgetPassword = prefs.getBoolean("forgetPassword",true);
        if(forgetPassword){
            Paper.book().destroy();
        }
